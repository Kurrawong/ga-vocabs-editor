...
// getLabel and PAGE_SIZE_ALL are auto-imported from the parent web layer's composables

const { isAuthenticated, isEnabled, login } = useGitHubAuth()
...
<template>
  <div class="py-8">
    <!-- Hero -->
    <div class="mb-10">
      <h1 class="text-3xl font-bold mb-3">GA Vocabularies</h1>
      <p class="text-lg text-muted max-w-3xl">
        A demonstration of vocabulary editing and publishing using K-Maker. Browse, edit, and
        manage SKOS vocabularies through a structured authoring workflow.
      </p>
      <div class="flex items-center gap-4 mt-4">
        <UButton to="/authoring" variant="soft" icon="i-heroicons-pencil-square">
          Learn about authoring
        </UButton>
        <UButton
          v-if="isEnabled && !isAuthenticated"
          variant="outline"
    ...
    <UCard v-else-if="viewMode === 'table' && paginatedSchemes.length">
      <UTable :data="tableData" :columns="tableColumns">
        <template #prefLabel-header="{ column }">
          <button class="flex items-center gap-1 hover:text-primary" @click="toggleSort('name')">
            {{ column.columnDef.header }}
            <UIcon v-if="sortBy === 'name'" :name="sortOrder === 'asc' ? 'i-heroicons-chevron-up' : 'i-heroicons-chevron-down'" class="size-4" />
          </button>
        </template>

        <template #conceptCount-header="{ column }">
          <button class="flex items-center gap-1 hover:text-primary" @click="toggleSort('concepts')">
            {{ column.columnDef.header }}
            <UIcon v-if="sortBy === 'concepts'" :name="sortOrder === 'asc' ? 'i-heroicons-chevron-up' : 'i-heroicons-chevron-down'" class="size-4" />
          </button>
        </template>

        <template #modified-header="{ column }">
          <button class="flex items-center gap-1 hover:text-primary" @click="toggleSort('modified')">
            {{ column.columnDef.header }}
            <UIcon v-if="sortBy === 'modified'" :name="sortOrder === 'asc' ? 'i-heroicons-chevron-up' : 'i-heroicons-chevron-down'" class="size-4" />
          </button>
        </template>

        <template #prefLabel-cell="{ row }">
          <NuxtLink :to="{ path: '/scheme', query: { uri: row.original.iri } }" class="text-primary hover:underline font-medium">
            {{ row.original.prefLabel }}
          </NuxtLink>
        </template>

        <template #conceptCount-cell="{ row }">
          <UBadge color="neutral" variant="subtle" size="sm">{{ row.original.conceptCount }}</UBadge>
        </template>

        <template #version-cell="{ row }">
          <UBadge v-if="row.original.version" color="primary" variant="subtle" size="xs">v{{ row.original.version }}</UBadge>
        </template>
        ...
        <template #validation-cell="{ row }">
          <UBadge v-if="row.original.validation?.conforms === true" color="success" variant="subtle" size="xs">Valid</UBadge>
          <UBadge v-else-if="row.original.validation?.errors" color="error" variant="subtle" size="xs">
            {{ row.original.validation.errors }} error{{ row.original.validation.errors !== 1 ? 's' : '' }}
          </UBadge>
          <UBadge v-else-if="row.original.validation?.warnings" color="warning" variant="subtle" size="xs">
